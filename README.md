# 난해하지만 그렇게 어렵지는 않은 PROBIE 언어, 0.3판 정의

# 1. 개요
- PROBIE 언어는 2차원 `필드`로 작성된 코드 위를 지속적으로 이동하는 `프로브`에 의해 연산이 수행되는 언어입니다. 코드 공간과 메모리 공간이 분리되어 있지 않으며, 모든 값이 문자 형태로 저장되며, 코드 자체가 실시간으로 수정되면서 작동한다는 점을 특징으로 합니다.
...
- 단, ASCII 코드표에서 0번부터 31번까지, 그리고 127번은 제어 문자로 입출력이 어렵기 때문에, 대신 다음의 문자들이 값을 가집니다.
  ...
  - `●` : 127
## 4.2. 특수한 문자
- 일부 문자들은 두 셀을 차지합니다. 프로브는 읽기 활동 도중 `\` 문자가 나오면 다음 문자까지 읽어 저장하게 됩니다. 이들은 `P`나 `I` 상태의 프로브가 개행 등의 작업을 처리하는 데에 사용됩니다. `I` 상태의 프로브에 이들 문자가 입력될 경우 입력 버퍼에 두 글자로 저장됩니다.
...
- `READ` 포인터는 커맨드가 아닌 문자는 별개로 판독하지 않습니다. `READ` 포인터의 경로상에 무슨 말을 써넣든지 그것은 여러분의 자유입니다!
## 5.1. 휘발성 커맨드
### 5.1.1. 조건문
...
- `I` : `WRITE` 포인터 위치에 사용자 입력 버퍼에 있는 값을 한 번에 한 글자씩 불러옵니다.
- `X` : `S`, `s`, `P`, `I` 동작을 중지합니다.
## 5.4. 주석
